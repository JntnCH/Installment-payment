  ...
  const runOperation = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (intent === "income" || intent === "expense") {
        const result = await apiRequest<{ message: string }>("/api/operations/transactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...entry, type: intent, amount: Number(entry.amount) }) });
