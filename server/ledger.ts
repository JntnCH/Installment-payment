...
import type { ContractInput, ContractUpdateInput, PartyInput, PartyUpdateInput, SchedulePaymentInput, ScheduleUpdateInput, TransactionInput, TransactionUpdateInput } from "../shared/ledgerSchemas";
...
function toParty(row: typeof parties.$inferSelect) {
  return {
    partyId: row.id,
...
function toContract(row: typeof contracts.$inferSelect) {
  return {
    contractId: row.id,
    partyId: row.partyId,
    title: row.title,
...
function toScheduleTransaction(row: typeof contractPaymentSchedules.$inferSelect) {
  return {
...
function toLedgerTransaction(row: typeof ledgerTransactions.$inferSelect) {
  return {
...
async function getOwnedParty(ownerId: number, partyId: string) {
  ...
  const [party] = await db.select().from(parties).where(and(eq(parties.ownerId, ownerId), eq(parties.id, partyId))).limit(1);
  return party ?? null;
}

async function getOwnedContract(ownerId: number, contractId: string) {
  const db = await requireDb();
  const [contract] = await db.select().from(contracts).where(and(eq(contracts.ownerId, ownerId), eq(contracts.id, contractId))).limit(1);
...
export async function getPartyLedger(ownerId: number, partyId: string) {
  ...
  const partyContracts = await db.select().from(contracts).where(and(eq(contracts.ownerId, ownerId), eq(contracts.partyId, partyId))).orderBy(desc(contracts.createdAt));
  const contractIds = partyContracts.map(contract => contract.id);
  const scheduleRows = contractIds.length
    ? await db.select().from(contractPaymentSchedules).where(and(eq(contractPaymentSchedules.ownerId, ownerId), inArray(contractPaymentSchedules.contractId, contractIds))).orderBy(asc(contractPaymentSchedules.dueDate))
    : [];
  const transactionRows = await db.select().from(ledgerTransactions).where(and(eq(ledgerTransactions.ownerId, ownerId), eq(ledgerTransactions.partyId, partyId))).orderBy(desc(ledgerTransactions.occurredAt));
...
export async function getContractLedger(ownerId: number, contractId: string) {
  ...
  const schedules = await db.select().from(contractPaymentSchedules).where(and(eq(contractPaymentSchedules.ownerId, ownerId), eq(contractPaymentSchedules.contractId, contractId))).orderBy(asc(contractPaymentSchedules.dueDate));
  const transactionRows = await db.select().from(ledgerTransactions).where(and(eq(ledgerTransactions.ownerId, ownerId), eq(ledgerTransactions.contractId, contractId))).orderBy(desc(ledgerTransactions.occurredAt));
...
export async function createLedgerContract(ownerId: number, input: ContractInput) {
  ...
  if (input.schedules.length) {
    await db.insert(contractPaymentSchedules).values(input.schedules.map(schedule => ({
...
export async function createLedgerTransaction(ownerId: number, input: TransactionInput) {
  ...
  const [row] = await db.select().from(ledgerTransactions).where(and(eq(ledgerTransactions.ownerId, ownerId), eq(ledgerTransactions.id, id))).limit(1);
...
export async function updateLedgerSchedule(ownerId: number, input: ScheduleUpdateInput) {
  const db = await requireDb();
  const [schedule] = await db.select().from(contractPaymentSchedules).where(and(eq(contractPaymentSchedules.ownerId, ownerId), eq(contractPaymentSchedules.id, input.id))).limit(1);
  ...
  const [updated] = await db.select().from(contractPaymentSchedules).where(and(eq(contractPaymentSchedules.ownerId, ownerId), eq(contractPaymentSchedules.id, input.id))).limit(1);
...
export async function updateLedgerTransaction(ownerId: number, input: TransactionUpdateInput) {
  const db = await requireDb();
  const [transaction] = await db.select().from(ledgerTransactions).where(and(eq(ledgerTransactions.ownerId, ownerId), eq(ledgerTransactions.id, input.id))).limit(1);
  if (!transaction) throw new Error("ไม่พบธุรกรรมที่เลือก");
  await db.update(ledgerTransactions).set({
  ...
  const [updated] = await db.select().from(ledgerTransactions).where(and(eq(ledgerTransactions.ownerId, ownerId), eq(ledgerTransactions.id, input.id))).limit(1);
...
export async function markSchedulePaid(ownerId: number, input: SchedulePaymentInput) {
  const db = await requireDb();
  const [schedule] = await db.select().from(contractPaymentSchedules).where(and(eq(contractPaymentSchedules.ownerId, ownerId), eq(contractPaymentSchedules.id, input.scheduleId))).limit(1);
  if (!schedule) throw new Error("ไม่พบงวดชำระที่เลือก");

  const paidAt = input.paidAt ?? new Date();
  await db.update(contractPaymentSchedules).set({ status: "paid", paidAmount: input.paidAmount.toFixed(2), paidAt }).where(eq(contractPaymentSchedules.id, input.scheduleId));
  ...
  const [updated] = await db.select().from(contractPaymentSchedules).where(eq(contractPaymentSchedules.id, input.scheduleId)).limit(1);
  return updated ? toScheduleTransaction(updated) : null;
}
